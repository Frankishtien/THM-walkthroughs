...
-   كل **كلمات السر والـ hashes** موجودة على:\
...
-   أي Login على:
    ...
    -   RDP\
        لازم الـ Service يسأل الـ DC:

    > "اليوزر ده حقيقي؟ والباسورد صح؟"

* * * * *

🔐 بروتوكولات الـ Authentication في الدومين
===========================================
...
| البروتوكول | الحالة |
| --- | --- |
| **Kerberos** | الافتراضي -- حديث -- آمن |
| **NetNTLM** | قديم -- للتوافق -- ضعيف |

📌 أغلب الشبكات شغالة بالاتنين 🤦‍♂️

* * * * *

🟣 أولًا: Kerberos Authentication (الأساسي)
===========================================

الفكرة ببساطة
-------------

> بدل ما تبعت باسوردك كل مرة،\
> **تثبت هويتك مرة → تاخد Tickets → تستخدمها بعد كده**

🎫 **Ticket = إثبات إنك بالفعل Authenticated**
...
-   مشفّرين بـ Key مشتق من **Password**
...
-   اليوزر **مايعرفش يفتحه**

-   الـ TGT جواه:

    -   Session Key (نسخة)

🧠 الـ KDC مش محتاج يخزن الـ Session Key\
لأنه يقدر يرجعه من الـ TGT

* * * * *

🔹 Step 2: طلب TGS (Service Ticket)
-----------------------------------

اليوزر عايز Service (مثلاً File Share):

يبعت للـ KDC:

-   Username

-   Timestamp مشفّر بـ Session Key
...
-   أو Computer Account\
    اللي السيرفيس شغال تحتها

* * * * *

🔹 Step 3: الدخول على السيرفيس
------------------------------
...
-   يفك التشفير باستخدام الـ Password Hash بتاعه

-   يتحقق من الـ Service Session Key
...
(كلها معتمدة على فهم التذاكر 🎫)

* * * * *

🔴 ثانيًا: NetNTLM Authentication
=================================
...
-   ولا الـ Hash\
    بيتبعَتوا على الشبكة

❌ لكن...

-   الـ Challenge + Response ممكن يتسرقوا
...
| نقطة | Kerberos | NetNTLM |
| --- | --- | --- |
| النوع | Ticket-based | Challenge-Response |
| الباسورد على الشبكة | ❌ | ❌ |
| الأمان | عالي جدًا | ضعيف |
| الهجمات | Tickets Abuse | Relay / Hash |
| الافتراضي | ✔ | ❌ (لكن موجود) |
...
-   NTLM Enabled = Red Flag 🚩

-   Kerberos Misconfig = كنز 💰

-   SPNs = Kerberoasting

-   krbtgt compromise = Domain pwned
...
<img width="1663" height="473" alt="image" src="https://github.com/user-attachments/assets/003c0941-cb03-4c39-8ca4-85ce88b27971" />
...
<details>
  <summary>Trees, Forests and Trusts</summary>



🏢 من Domain واحد → Enterprise كاملة
====================================
...
-   UK → يدير uk.thm.local فقط

-   US → يدير us.thm.local فقط
...
👑 Admin Groups في الـ Tree
---------------------------

### 1️⃣ Domain Admins

-   صلاحيات **داخل الدومين فقط**

-   UK Domain Admin ≠ US Domain Admin

### 2️⃣ Enterprise Admins ⚠️

-   صلاحيات على:\
    🟢 **كل الدومينات**

-   أخطر حساب في الـ Enterprise

💀 اختراق Enterprise Admin = نهاية الغابة 🌲
...
> سماح بين دومينين إن يوزر من واحد\
> يقدر يتأذن له يدخل موارد التاني
...
✔ Users من **BBB**\
→ يقدروا يدخلوا Resources في **AAA**

❌ العكس؟ لا

📌 **اتجاه الثقة عكس اتجاه الدخول**
...
🧠 الصورة الكبيرة (تحفظها)
==========================

| المفهوم | معناه |
| --- | --- |
| Domain | وحدة إدارة |
| Tree | Domains بنفس الاسم |
| Forest | Trees بأسماء مختلفة |
| Domain Admin | سلطان داخل دومينه |
| Enterprise Admin | إله الغابة |
| Trust | سماح مش إجبار |

* * * * *

🔥 من منظور أمني (Pentester 💀)
===============================
